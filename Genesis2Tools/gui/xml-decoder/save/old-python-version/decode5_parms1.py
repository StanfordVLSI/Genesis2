...
def process_parameter_item(firstline, path):
    ...
    while (input_lines):
        ...
        hatype    = re.search("^\s*<(Array|Hash)Type>\s*$", line);
        if (hatype):
            parmblock = parmblock + process_hash_or_array_type(path, hatype.group(1)); # "Array" or "Hash"
            if debug5: print "ppi(): back from calling phat(); next input line = %s" % input_lines[0]
            continue;
...
#    printdie("process_hash_or_array_type not implemented yet.");
    ...
    wore = "^\W*<ArrayItem><Val>(.*)</Val></ArrayItem>\W*$"

    # E.g. "<ArrayItem><Val>3</Val></ArrayItem>"
    weird_oneliner = re.compile(wore);   # "/ArrayType" or "/HashType"
    ...
    while (input_lines):
        ...
        if weird_oneliner.search(line): # E.g. "<ArrayItem><Val>3</Val></ArrayItem>"
            val = weird_oneliner.search(line).group(1);
            subpath = "%s[%d]" % (path,i); i = i + 1;
        ...
        elif new_item.search(line):       # i.e. "HashItem" or "ArrayItem"
            block.append("#" + line);   # Comment-out processed lines.
