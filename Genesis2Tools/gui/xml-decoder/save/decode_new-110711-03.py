...
def preprocess_subinsts():
    ...
    print "hello i am process_subinsts";
    sys.exit(0);
...
####################################################################################################
#PASS1: SUBINST, HASHKEYS

#if ($do_pass[++$passno]) {
#    print "# pass$passno - preprocess subinstance items\n";
#    $print_savelines = $print_pass[$passno];  # If =1 then print lines as they are processed.
#    preprocess_subinsts();
#    @input_lines = @output_lines; @output_lines = ();

passno = passno + 1
if do_pass[passno]:
    print "# pass%d - preprocess subinstance items" % passno;
    print_savelines = print_pass[passno];  # If == 1 then print lines as they are processed.
    preprocess_subinsts();
    input_lines = output_lines; output_lines = []; # Reset input lines.
