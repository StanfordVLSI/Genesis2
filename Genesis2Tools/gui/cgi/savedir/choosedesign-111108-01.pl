...
#my @fv_pairs = split /\&/ , $ENV{QUERY_STRING);
#my $DELETE = 0;
#foreach my $pair (@fv_pairs) {
#    if($pair=~m/DELETE=(.*)/) {                          # E.g. "(newdesign)=(my_design_name)"
#        $DELETE = $1;
#    }
#}

my $qs = $ENV{QUERY_STRING};
#print embed_alert_script("okay here we are");
#print embed_alert_script($qs);

my %INPUT = get_input_parms($qs);

my $default_design = "fooby"; # No default design!

if ($INPUT{DELETE}) {
    my $delete = $INPUT{"DELETE"};
    my $design = $INPUT{"DESIGN"};
#    print embed_alert_script("gotta delete $delete");
#    print embed_alert_script("and reopen design $design");

    my $cmd = "mv $delete $delete.deleteme";
#    print embed_alert_script($cmd);
    my $err = `$cmd`;
#    print embed_alert_script($err);
...
#         '  alert("ima gonna delete " + filename);',
#         '  alert("and reopen design " + design);',
#         '  alert("but how?");',
...
#    my ($default_design,$default_config) = ("fooby", "barby"); # No default design!
	...
	my @list = split(" ", $designs->{$k});
        ...
        my $source_dir = `cat ../designs/$k/__SOURCEDIR__`;
...
#        print embed_alert_script("source dir for $k is $source_dir");
	...
	foreach my $design (@list) {
	    my $fullpath = "../designs/$k/$design";
            my $ls = "ls -l $source_dir/$design";
#            print embed_alert_script($ls);
            my $ls_out = `$ls`;
#            print embed_alert_script($ls_out);
#            return;

            my $deletable = (-e "$source_dir/$design") ? 0 : 1;

#            if (-e "$source_dir/$design") {
