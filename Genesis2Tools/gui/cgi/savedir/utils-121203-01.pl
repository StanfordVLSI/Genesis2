...
#        my $pwd = `pwd`;
#        print embed_alert_script("Found makefile \"$pwd/$makefile\"\n");

        my $sourcedir = `cat $designdir/__SOURCEDIR__`;
        $sourcedir =~ s/^\s*//; # Eliminate leading spaces
        $sourcedir =~ s/\s*$//; # Eliminate trailing spaces
        print embed_alert_script("Found source dir \"$sourcedir\"");
        

#        my $makefile_contents = `head $makefile`;
#        print embed_alert_script($makefile_contents);
        
        my $gui_info = `make -f $sourcedir/Makefile gui_info`;
        print embed_alert_script("Found gui info:\n$gui_info");

#        my $gui_info = `egrep '^###GUI' $makefile`;
#        my $gui_command = `grep GUI $makefile`;
#        print embed_alert_script("Found gui command:\n$gui_command");
#
#        my $gui_command =~ s/###GUI:\s*//;
#        print embed_alert_script("Found gui command:\n$gui_command");
#
#        my $command_results = `$gui_command`;
#        print embed_alert_script("Command result:\n$command_results");
    }
...
#    print embed_alert_script("calling $gencmd\n");
    ...
    if ($DBG) { alert($alert_msg); }

    if (($alert_msg =~ /Usage/) || ($alert_msg =~ /ERR/)) {
      alert($alert_msg);
      print "Genesis2 driver returned the following error:<br>\n";
      print "$alert_msg<br>\n";
      exit;
    }
}
