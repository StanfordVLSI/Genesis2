...
#    print "\nWill check to see if port $port is available\n\n";
    ...
    do {
        ...
        if ($portbusy =~ /^\s*$/) {
            print "Yay port $port is (still) free\n\n";
            $stop = 1;
        }
        else {
    ...
    if (! ($gotport && $gotgui)) {
        ...
        # redo the call, this time with better info
        chdir("bin"); # Sure why not?
        my $cmd = "./stewie.pl gui=\"$curdir\" port=$port";
        print "Resubmitting call as '$cmd'\n\n";
#        `./stewie.pl gui=\"$guidir\" port=$port`;
        my $err = exec($cmd);

        # This generally never gets executed because exec() kills the process for us.
        print "stewie said \"$err\"\n\n";
        print "Bye now...\n\n";
        exit(0);
    }
#    ########################################################################
#    # Ooh I like this one.  Set an environment variable with relevant
#    # process information and then later we can find it with a simple
#    # "ps ex"
#    my $curdir = `pwd`; chomp($curdir);
#    print "booooostew\n\n";
#    
#    $ENV{STEWIE_INFO} = "$pid $curdir $port END_STEWIE_INFO";

###    my $cat = `/bin/csh -c /bin/cat &`; # Really?  cat?
###    print "cat did this: $cat\n\n";
###    print `ps ax | grep cat | grep -v notifi`;
###    exit;
##
###    my $tail = `tail -f /dev/null > /dev/null &`;
###    print "tail did this: $tail\n\n";
###    print `ps ax | grep tail`;
###    exit;
##
###    my $tail = `tail -f /dev/null > /dev/null &`;
##    my $stinfo = `./bin/stewie_info.sh $pid $curdir $port`;
##    print "stinfo did this: $stinfo\n\n";
##    print `ps ax | grep stewie_info`;
##    exit;
