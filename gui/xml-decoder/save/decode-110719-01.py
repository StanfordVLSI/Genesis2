...
def final_pass():
	...
	# => 'path.BaseModuleName = "top";'
		...
		print("%s.%s = \"%s\";" % (printpath(nestlevel, tag), tag1, data));

        elif (parmhint):                                # "^\#PARM (.*)"

            # E.g. #PARM MODE = "VERIF"
           
            path = printpath(nestlevel, tag);
...
#	    tag[nestlevel] = find_sub.group(1);  # FOO NESTLEVEL ELIMINATION
            tag.append(find_sub.group(1));  # FOO NESTLEVEL ELIMINATION


	    print "%s = new Object();" % printpath(nestlevel, tag);

	elif (find_obj):                                           # "^\#OBJECT (.*)"
	    print "%s.%s;" % (printpath(nestlevel,tag), find_obj.group(1));

	elif (find_ipath):                                         # "^\#IPATH (.*)"
	    print "%s.%s;" % (printpath(nestlevel,tag), find_ipath.group(1));
...
#                tag[nestlevel] = tagname;

                tag.append(find_sub.group(1));  # FOO NESTLEVEL ELIMINATION
                


           #tag[nestlevel+1] = tagname; nestlevel = nestlevel + 1;
            print "%s = new Object();" % printpath(nestlevel, tag);
	...
	# Close-tag on a line by itself
            ...
            if debug: print "fp(): nestlevel before processing close-tag = %d" % nestlevel;
            if debug: print "path now = %s" % printpath(nestlevel,tag)
	    ...
	    # Correct fix: use separate arrays to track tags, pathnames.
            ...
            print ""
            print "FOO1 %s" % nestlevel
            print "FOO2 %s" % (len(tag)-1)
            print ""
	    ...
	    nestlevel = nestlevel - 1;
            ...
            # FOO NESTLEVEL ELIMINATION

            if (nestlevel >=0):  
                if debug: print "fp(): nestlevel after processing close-tag = %d" % nestlevel;
                if debug: print "path now = %s" % printpath(nestlevel,tag)
...
def printpath(plevel, tag):
    """E.g. printpath(2, ["cgtop","DUT","p0"]) => "cgtop.DUT"""

    debugpp = 1
    if debugpp: print "\nEntering printpath(%s, %s)" % (plevel,tag);

    rval = tag[0];
   
    print ""
    print "FFFOO1 %s" % plevel
    print "FFFOO2 %s" % len(tag)
    print ""


#    print "HEY %d => " % plevel,; print range(1,plevel);
#    print "HEY %d => " % plevel,; print range(1,2,1);
#    print "HEY %d => " % plevel,; print range(1);
#
    for i in range(1,plevel+1):                    # perl: for (i=1; i <= plevel; i++)
        rval = "%s.%s" % (rval, tag[i]);
        if debugpp: print "    pp says: (i,rval,tag[i] = %d,%s,%s" % (i,rval,tag[i]);

    if (debugpp): print "    printpath(%s, %s) = %s\n" % (plevel,tag,rval);
    return rval;
