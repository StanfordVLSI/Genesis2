...
def process_parameter_item(firstline, path):
   #print "i am process_parameter_item(\n    \"%s    \"%s\n)" % (firstline, path);
	...
	# ALSO POSSIBLE TO HAVE one-line "<ArrayType></ArrayType>" (no ArrayItem's)
        ...
        rval = check_for_simple_parms(line);
        if (rval == "continue"): continue;
        if (rval == "break"):    break;
	...
	#</ParameterItem> => print parmname, parmval, parmblock; done.
        found = pi_search(line, parmname, parmval, simpleparm, found_ipath, savelines);
        if (found): 
...
def check_for_simple_parms(line):
...
def valsearch(line):
    # <Val>0</Val> => return parmval "0" (including the double-quotes)
...
def ipsearch(line):
    # "<InstancePath>cgtop.DUT.p0</InstancePath>" => return instance path "cgtop.DUT.p0"

    ipsearch  = re.search("^\s*<InstancePath>([^<]+)<\/InstancePath>", line);
    if (ipsearch): # "^\s*<InstancePath>([^<]+)<\/InstancePath>"
        return (1,ipsearch.group(1));
    else:
        return (0,0);
